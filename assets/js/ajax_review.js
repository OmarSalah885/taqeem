document.addEventListener('DOMContentLoaded', () => {
    ...
    function bindEditForm(form) {
        ...
        // Initialize deleteImages from existing hidden inputs
        ...
        // Update hidden inputs for delete_images[]
        function updateDeleteImagesInputs() {
            ...
            console.log(`Updated delete_images[] inputs for review ID: ${reviewId}, inputs:`, Array.from(form.querySelectorAll('input[name="delete_images[]"]')).map(input => input.value));
        }
        ...
        form.addEventListener('submit', (e) => {
            e.preventDefault();
            if (isSubmitting) {
                console.log(`Submission blocked for review ID: ${reviewId}, already in progress`);
                return;
            }
            isSubmitting = true;
            updateDeleteImagesInputs(); // Ensure hidden inputs are current
            const formData = new FormData(form);
            const formDeleteImages = Array.from(formData.getAll('delete_images[]'));
            console.log('Submitting edit form for review:', reviewId, 'with delete_images:', Array.from(deleteImages), 'FormData delete_images:', formDeleteImages);
            if (deleteImages.size > 0 && formDeleteImages.length === 0) {
                console.error(`Mismatch: deleteImages has ${deleteImages.size} items, but FormData has none. Retrying input update.`);
                updateDeleteImagesInputs();
                formData.delete('delete_images[]');
                deleteImages.forEach(imgId => formData.append('delete_images[]', imgId));
            }
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (reviewElement) {
                        ...
                        const newEditForm = reviewElement.querySelector(`#editForm-${data.review_id}`);
                        if (newEditForm) {
                            newEditForm.style.display = 'none';
                            bindStarRating(`#editForm-${data.review_id} .addReview_stars`);
                            bindEditForm(newEditForm); // Rebind the form
                            console.log('Bound edit form:', data.review_id);
                        }
                        reviewElement.scrollIntoView({ behavior: 'smooth' });
                    }
                    form.style.display = 'none';
                    deleteImages.clear(); // Clear deleteImages after successful submission
                    updateRatings(data.avg_rating, data.total_reviews, data.ratings_counts);
                } else {
            ...
            .finally(() => {
                isSubmitting = false;
                console.log(`Submission completed for review ID: ${reviewId}`);
            });
        });
